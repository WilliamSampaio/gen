...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column('fs_id', sa.String(8), nullable=False, unique=True),
        sa.Column('name', sa.String(100), nullable=False),
        sa.Column('born_in', sa.Date()),
        sa.Column('died_in', sa.Date()),
        sa.Column('gender', sa.CHAR()),
        sa.Column('father_id', sa.Integer, sa.ForeignKey('tree.id')),
        sa.Column('mother_id', sa.Integer, sa.ForeignKey('tree.id')),
        sa.Column(
            'inserted_in',
            sa.DateTime(),
            nullable=False,
